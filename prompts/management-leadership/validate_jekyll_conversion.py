...
class JekyllValidator:
    ...
    def validate_prompt_file(self, file_path: Path) -> Dict:
        ...
        if front_matter.get('category') not in valid_categories:
            self.errors.append(f"{file_path}: Invalid category '{front_matter.get('category')}', must be one of {valid_categories}")
    ...
    def print_report(self) -> None:
        ...
        if self.errors:
            ...
            for i, error in enumerate(self.errors):
                if i < 20:  # Limit error output for readability
                    print(f"  ❌ {error}")
                elif i == 20:
        ...
        if self.warnings:
            ...
            for i, warning in enumerate(self.warnings):
                if i < 20:  # Limit warning output for readability
                    print(f"  ⚠️  {warning}")
                elif i == 20:
