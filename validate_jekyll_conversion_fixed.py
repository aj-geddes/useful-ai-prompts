...
class JekyllValidator:
    ...
    def __init__(self, prompts_dir: str = "docs/_prompts", categories_dir: str = "docs/_categories"):
        ...
        self.warnings = []
        self.stats = {}
        # Dynamically load valid categories from filesystem
        self.valid_categories = self._load_valid_categories()
    
    def _load_valid_categories(self) -> Set[str]:
        """Load valid categories from category definition files."""
        if not self.categories_dir.exists():
            return set()
        
        return {f.stem for f in self.categories_dir.glob('*.md')}
    ...
    def validate_prompt_file(self, file_path: Path) -> Dict:
        ...
        if front_matter.get('category') not in self.valid_categories:
            self.errors.append(f"{file_path}: Invalid category '{front_matter.get('category')}', must be one of {sorted(self.valid_categories)}")
