...
function initSocialSharing(config) {
  ...
  setupSocialButton(
    "twitterShare",
    shareUrls.twitter(url, title, hashtags, via),
    "twitter",
  );
  setupSocialButton(
    "linkedinShare",
    shareUrls.linkedin(url, title, description),
    "linkedin",
  );
  setupSocialButton("redditShare", shareUrls.reddit(url, title), "reddit");
  setupSocialButton(
    "whatsappShare",
    shareUrls.whatsapp(url, title, description),
    "whatsapp",
  );
  setupSocialButton(
    "telegramShare",
    shareUrls.telegram(url, title, description),
    "telegram",
  );
  setupSocialButton(
    "emailShare",
    shareUrls.email(url, title, description),
    "email",
  );
...
function addUTMParams(url, source) {
  ...
  urlObj.searchParams.set(
    utmConfig.campaign_param || "utm_campaign",
    utmConfig.default_campaign || "social_share",
  );
...
async function loadGitHubStats() {
  try {
    const response = await fetch(
      "https://api.github.com/repos/aj-geddes/useful-ai-prompts",
    );
...
function loadPopularPrompts() {
  ...
  const popularPrompts = [
    ...
    {
      title: "Business Analyst Excellence",
      category: "Business",
      views: "1.8K",
    },
    {
      title: "Fullstack Developer Architect",
      category: "Technical",
      views: "1.5K",
    },
  ];
  ...
  if (container) {
    container.innerHTML = popularPrompts
      .map(
        (prompt) => `
...
function generateCustomShareMessage(platform) {
  const messages = {
    twitter:
      "Just discovered this amazing AI prompt library! 🤖 Over 278+ professional prompts across 18 categories. Perfect for boosting productivity! #AI #Productivity",
    linkedin:
      "Sharing this comprehensive AI prompt library that's been incredibly valuable for professional workflows. Great resource for anyone working with AI assistants.",
    reddit:
      "Found this free library of 278+ professional AI prompts - perfect for anyone using ChatGPT, Claude, or other AI tools for work",
  };
...
function setupSharingIncentives() {
  ...
  document.addEventListener("social_share", () => {
    ...
    if (shareStreak === 3) {
      ...
      showAchievement(
        "Community Builder",
        "Amazing! You've shared 10 prompts!",
      );
    }
  });
}
