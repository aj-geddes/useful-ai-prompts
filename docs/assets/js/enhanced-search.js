...
(function () {
  ...
  function createSearchDropdown() {
    ...
    const inputContainer =
      searchInput.closest(".search-input-wrapper, .search-input-container") ||
  ...
  function setupSearchHandlers() {
    ...
    if (tagFilter) {
      tagFilter.addEventListener("change", (e) => {
        currentFilters.tags = Array.from(e.target.selectedOptions).map(
          (opt) => opt.value,
        );
        const query = searchInput.value.trim();
        if (query) performSearch(query);
  ...
  function showSmartSuggestions(query) {
    ...
    if (suggestions.prompts.length > 0) {
      ...
      html +=
        '<div class="suggestion-header"><i class="fas fa-file-alt"></i>Prompts</div>';
    ...
    if (suggestions.categories.length > 0) {
      html += '<div class="suggestion-section">';
      html +=
        '<div class="suggestion-header"><i class="fas fa-folder"></i>Categories</div>';
    ...
    if (suggestions.tags.length > 0) {
      html += '<div class="suggestion-section">';
      html +=
        '<div class="suggestion-header"><i class="fas fa-tags"></i>Tags</div>';
      suggestions.tags.forEach((tag, index) => {
        const globalIndex =
          suggestions.prompts.length + suggestions.categories.length + index;
  ...
  function showRecentSearches() {
    ...
    html += '<div class="suggestion-section">';
    html +=
      '<div class="suggestion-header"><i class="fas fa-history"></i>Recent Searches</div>';
  ...
  function generateSmartSuggestions(query) {
    ...
    const promptMatches = searchIndex
      .map((item) => {
        let score = 0;
  ...
  function handleKeyNavigation(e) {
    ...
    switch (e.key) {
      case "ArrowDown":
        e.preventDefault();
        dropdownSelectedIndex = Math.min(
  ...
  function buildMinimalSearchIndex() {
    const promptCards = document.querySelectorAll(
      ".prompt-card, .category-card",
    );
    searchIndex = Array.from(promptCards)
      .map((card) => {
  ...
  function searchPromptsOptimized(query) {
    ...
    console.log(
      `Search completed in ${endTime - startTime}ms for ${sortedResults.length} results`,
    );
  ...
  function applyFilters(item) {
    ...
    if (currentFilters.sector) {
      ...
      if (
        currentFilters.sector === "high-value" &&
        !highValueSectors.includes(item.category)
      ) {
        return false;
      }
    }
                  ...
                  result.tags && result.tags.length > 0
                    ...
                    <div class="result-tags">
                        ${result.tags
                          ...
                          .map(
                            (tag) =>
                              `<span class="tag" onclick="searchByKeyword('${tag}')">${tag}</span>`,
                          )
                          .join("")}
  ...
  function getRelatedPrompts(currentPrompt, count = 4) {
    ...
    return searchIndex
      ...
      .map((prompt) => {
        ...
        // Tag matches
        const commonTags = prompt.tags.filter((tag) =>
        ...
        const commonWords = currentWords.filter(
          (word) => promptWords.includes(word) && word.length > 3,
        );
