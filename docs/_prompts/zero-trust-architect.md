...
- GPT-4+
date: '2026-02-28'
description: This prompt activates a zero trust architecture specialist who designs
  and guides implementation of identity-centric security models aligned to NIST SP
  800-207 and the CISA Zero Trust Maturity Model. The expert replaces implicit network-based
  trust with continuous verification of identity, device posture, and context before
  granting resource access. Outputs include architecture designs, maturity assessments,
  implementation roadmaps, and technology selection guidance.
layout: prompt
use_cases:
- Ideal Scenarios:**
...
- Evaluating and selecting zero trust technology components (ZTNA, CASB, identity
  provider, PAM)
- Organizations with no existing identity management — zero trust requires a solid
  identity foundation first
complexity: advanced
